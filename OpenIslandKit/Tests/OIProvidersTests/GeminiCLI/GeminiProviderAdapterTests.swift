...
// MARK: - GeminiCLIProviderAdapterTests

@Suite(.tags(.gemini))
struct GeminiCLIProviderAdapterTests {
    @Test
    func `conforms to ProviderAdapter`() {
        let adapter = GeminiCLIProviderAdapter(socketPath: uniqueSocketPath())
        let pa: any ProviderAdapter = adapter
        #expect(pa.providerID == .geminiCLI)
    }

    @Test
    func `has correct provider metadata`() {
        let adapter = GeminiCLIProviderAdapter(socketPath: uniqueSocketPath())
        #expect(adapter.providerID == .geminiCLI)
        #expect(adapter.transportType == .hookSocket)
        #expect(adapter.metadata.displayName == "Gemini CLI")
    }
    ...
    func `can register with ProviderRegistry`() async {
        ...
        let found = await registry.adapter(for: .geminiCLI)
        #expect(found != nil)
        #expect(found?.providerID == .geminiCLI)
    }
...
// MARK: - GeminiCLIProviderAdapterLifecycleTests

@Suite(.tags(.gemini, .socket), .serialized)
struct GeminiCLIProviderAdapterLifecycleTests {
    @Test(.timeLimit(.minutes(1)))
    func `start and stop lifecycle`() async throws {
        let path = uniqueSocketPath()
        let adapter = GeminiCLIProviderAdapter(socketPath: path)
    ...
    func `events stream emits normalized events from socket`() async throws {
        ...
        let payload = #"{"session_id":"s1","hook_event_name":"SessionStart","cwd":"/tmp"}"# + "\n"
...
private func uniqueSocketPath() -> String {
    "/tmp/oi-test-gemini-\(UUID().uuidString.prefix(8)).sock"
}
