#!/usr/bin/env python3
"""Open Island hook script for Gemini CLI.

Forwards hook events to the Open Island macOS app via Unix domain socket.
For BeforeTool events, blocks waiting for the app's approve/deny decision.

Protocol:
    - Reads JSON from stdin (Gemini CLI hook event)
    - Connects to Unix domain socket at SOCKET_PATH
    - Sends event JSON followed by newline
    - For BeforeTool: reads decision JSON from socket, outputs to stdout
    - For all other events: outputs {} to stdout
    - Exit 0 on success; never blocks Gemini CLI on failure
"""
...
SOCKET_PATH = "/tmp/open-island-gemini.sock"
...
# The hook can deny execution or rewrite arguments before the tool runs.
BLOCKING_EVENTS = {"BeforeTool"}
...
def main() -> None:
    ...
    is_blocking = hook_name in BLOCKING_EVENTS
